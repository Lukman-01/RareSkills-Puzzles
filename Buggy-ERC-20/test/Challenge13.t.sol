...
contract ApprovalBugTest is Test {
    ...
    function testApprovalAndValidationBugs() public {
        ...
        // allowance is at slot 4 (name=0, symbol=1, decimals=immutable, totalSupply=2, balanceOf=3, allowance=4)
        // Allowance slots: keccak256(abi.encode(spender, keccak256(abi.encode(owner, 4))))
        bytes32 bobAliceSlot = keccak256(abi.encode(alice, keccak256(abi.encode(bob, uint256(4)))));
        bytes32 aliceBobSlot = keccak256(abi.encode(bob, keccak256(abi.encode(alice, uint256(4)))));
